...
def add_medium_fog(image):
    ...
    # 将雾层和图片按照雾度系数混合，得到添加中雾效果的图片
    fogged_image = cv2.addWeighted(image, 1 - fog_coeff, fog_layer, fog_coeff, 0)
    # 返回添加中雾效果的图片
    return fogged_image

# 定义一个文件夹路径，存放要处理的图片
folder_path = r"C:\Users\Administrator\Desktop\test\cla\test"
# 遍历文件夹下的所有文件
for file in os.listdir(folder_path):
    ...
    if file_ext.lower() in [".jpg", ".jpeg"]:
        ...
        # 给图片添加中雾效果
        fogged_image = add_medium_fog(image)
        # 保存添加中雾效果的图片到原来的路径，名称后面加上_fogged
        cv2.imwrite(file_path.replace(file_name, file_name + "_fogged"), fogged_image)
        # 打印出处理成功的信息
        print(f"Added medium fog effect to {file}")
    else:
        # 如果不是，打印出该文件的文字和后缀
